...
Provision and [monitor](https://github.com/ktsouvalis/akropolis-monitor) a highly-available 3-node/single-node [Authentik](https://goauthentik.io) cluster over SSH.

akropolis turns three fresh Ubuntu 24.04 VMs into a production-grade Authentik identity provider cluster: PostgreSQL 16 with Patroni auto-failover over etcd, per-node HAProxy connection routing, nginx TLS termination, and a keepalived VRRP virtual IP, following a battle-tested implementation guide. It runs from your workstation, needs nothing installed on the nodes beforehand, and ends by emitting a ready-to-use config for its monitoring companion.

Certificates from any source: self-signed, any ACME CA (Let's Encrypt), or externally issued files.

## Architecture deployed

Akropolis orchestrates a robust, highly-available identity infrastructure powered by the following open-source technologies:

| Component | Role | Link |
| :--- | :--- | :--- |
| <img src="https://docs.goauthentik.io/img/icon.png" width="24" /> **Authentik** | Identity Provider | [goauthentik.io](https://goauthentik.io) |
| <img src="https://upload.wikimedia.org/wikipedia/commons/2/29/Postgresql_elephant.svg" width="24" /> **PostgreSQL** | Database Backend | [postgresql.org](https://www.postgresql.org/) |
| <img src="https://patroni.readthedocs.io/en/latest/_images/patroni-logo.png" width="24" /> **Patroni** | PostgreSQL HA | [patroni.readthedocs.io](https://patroni.readthedocs.io/) |
| <img src="https://raw.githubusercontent.com/cncf/artwork/main/projects/etcd/icon/color/etcd-icon-color.svg" width="24" /> **etcd** | Distributed Lock & Config | [etcd.io](https://etcd.io/) |
| <img src="https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/haproxy.png" width="24" /> **HAProxy** | Postgres Router | [haproxy.org](https://www.haproxy.org/) |
| <img src="https://upload.wikimedia.org/wikipedia/commons/c/c5/Nginx_logo.svg" width="24" /> **NGINX** | TLS Termination & LB | [nginx.org](https://nginx.org/) |
| <img src="https://www.keepalived.org/assets/keepalived-logo.png" width="24" /> **Keepalived** | VRRP Virtual IP | [keepalived.org](https://www.keepalived.org/) |
| <img src="https://upload.wikimedia.org/wikipedia/commons/4/4e/Docker_%28container_engine%29_logo.svg" width="24" /> **Docker** | Container Runtime | [docker.com](https://www.docker.com/) |
| <img src="https://upload.wikimedia.org/wikipedia/commons/c/c3/Python-logo-notext.svg" width="24" /> **Python** | CLI Automation Engine | [python.org](https://www.python.org/) |
| <img src="https://upload.wikimedia.org/wikipedia/commons/a/ab/Logo-ubuntu_cof-orange-hex.svg" width="24" /> **Ubuntu** | Base OS (24.04) | [ubuntu.com](https://ubuntu.com/) |
...
| base / etcd / patroni / haproxy | ✅ implemented, tested |
| tls: `none` / `self_signed` / `acme` (staging) / `import` | ✅ implemented, tested |
| nginx-keepalived (incl. ACME finalization) | ✅ implemented, tested (ACME finalized against Let's Encrypt production) |
| authentik | ✅ implemented, tested (compose mirrors the production file verbatim) |
| restore | ✅ implemented, tested (optional, skipped unless `restore.sql_file` is set) |
| handoff | ✅ implemented, tested (emits the real ak-monitor schema) |

**Both topologies have been provisioned end-to-end against real VMs.**

## Install

Download the single-file executable from the
[latest release](https://github.com/ktsouvalis/akropolis/releases/latest):
...
[zipapp](https://docs.python.org/3/library/zipapp.html) carrying akropolis and
its pure-Python dependencies, no virtualenv, no `pip`, no root, nothing to
uninstall later. Drop it in `~/.local/bin` or `/usr/local/bin` if you want it on
`PATH`.

The three apt packages are paramiko's compiled dependencies. They are
deliberately *not* bundled: `zipimport` cannot load extension modules out of a
zip, and freezing a crypto library inside a release artifact is the wrong
...
```bash
python3 -c "import zipfile;print(zipfile.ZipFile('akropolis').read('BUNDLE-MANIFEST.txt').decode())"
```

Verify the download against `SHA256SUMS` from the same release. Builds are
reproducible given the same commit **and the same Python minor version**.
Releases are built on Python 3.10, the floor in `requires-python`:
...
`typing_extensions` only below 3.11, so an artifact built on 3.12 omits it and
fails on a 3.10 host. `BUNDLE-MANIFEST.txt` records which interpreter built a
given file. Building on something newer is fine for local use, the script
says so and the result runs, it just will not match the release checksum, and
should not be handed to anyone else.
...
provision:
  state_file: .state/example-site.json
...
nodes:                      # exactly 3; first is bootstrap_leader by default
  - { name: ak-node-1, ip: 10.0.0.11, bootstrap_leader: true }
  - { name: ak-node-2, ip: 10.0.0.12 }
  - { name: ak-node-3, ip: 10.0.0.13 }

network:
  vip: 10.0.0.10            # must share the nodes' /24 (VRRP needs L2 adjacency)
  interface: ens18
...
postgres:
  extra_pg_hba: []          # site-specific lines appended to the generated pg_hba
                            # e.g. "host postgres postgres 10.20.0.50/32 scram-sha-256"
base:
...
Checks per node: SSH reachability and root/sudo, OS release (warn if not Ubuntu 24.04), interface exists with the expected MTU, ≥ 20 GB free on `/`, all 14 required ports free (80, 443, 2379, 2380, 5432, 5000, 5001, 8008, 9000, 9080, 9081, 9300, 9301, 9443), no existing cluster artifacts (`/etc/patroni`, running etcd/haproxy/nginx/authentik containers, enabled keepalived) when `refuse_existing` is set, and an inter-node ping with the DF bit at the expected MTU (catches VXLAN overhead misconfiguration before it becomes a mysterious replication stall). Cluster-wide: clock skew ≤ 5 s, VIP not answering (nothing may own it yet), and DNS for `tls.hostname` resolving to *something*: a hard failure for `acme`, a warning otherwise.

That last one is deliberately loose. akropolis reports whether the answer happens to be the VIP, but does not require it: a public hostname pointing at a public address that a firewall forwards `:80/:443` to the VIP is a correct deployment, and a private VIP can never appear in a public record anyway. Resolving to the VIP would not prove reachability either. What breaks ACME is a name that resolves to nothing, and that is what blocks. **Whether the address it does resolve to actually reaches the VIP is yours to verify**: akropolis cannot see a NAT hop from inside the cluster.
...
The emitted monitor config always sets `scheme.verify_tls: false`, whatever `tls.provider` is. The monitor identifies individual nodes, so it dials them by IP, and no certificate the cluster can hold names an IP; `true` would fail the handshake on every node and paint a healthy cluster as nginx-down / keepalived-FAULT / VIP-unreachable. Set it to `true` yourself only if your nodes present certificates naming exactly what the monitor dials.

**Provisioned before v0.16.11?** Two things that version fixes live outside the code, and no later run reconciles them on their own:

```bash
# 1. on the workstation: the monitor config already emitted
sed -i 's/verify_tls: true/verify_tls: false/' config.<site>.monitor.yml

# 2. on every node: the UFW rule the base phase never wrote
sudo ufw allow from <monitor-ip> to any port 8080 proto tcp comment 'ak-monitor stub_status'
```

Neither needs a service restart; restart the monitor TUI and the panels come back. `--replay base` would write the port rule too, but as one combined multi-port rule alongside the standalone one added here; tidy the leftover if you go that route.

Triage from the monitor host, if a panel is red and you want to know which layer:

```bash
curl -s  https://<node-ip>/monitor            # rc 60 → cert can't verify: verify_tls is true
curl -s --max-time 5 http://<node-ip>:8080/nginx_status
                                              # rc 28 → firewall dropping (silent, nothing logged)
                                              # rc  7 → nothing listening
                                              # rc  0 → the port is fine, look at the monitor config
```
...
If public TLS is terminated upstream (e.g. a Traefik instance holding a HARICA wildcard, with the cluster reachable only through it), the intended setup is `tls.provider: self_signed`: the internal VIP serves a self-signed cert, and the external proxy forwards to it with certificate verification disabled. Preflight's DNS check never required the VIP in the first place, and is warn-only for `self_signed` regardless, so a public hostname that resolves to the proxy rather than the VIP does not block.
...
This repository's commit history begins at v1.0.1. History prior to this point is archived internally; the `v1.0.0` and `v1.0.1` tags and release artifacts are unchanged and continue to resolve as before.
