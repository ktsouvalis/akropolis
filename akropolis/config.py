...
def load(path: str | Path) -> SiteConfig:
    ...
    # --- authentik ---
    # An unquoted `tag: 2026.10` is parsed by YAML as the float 2026.1, and
    # str() would turn that into "2026.1" — a real tag, a different release,
    # pinned across every node without a word. Caught here rather than
    # discovered by `docker pull`.
    raw_tag = _get(raw, "authentik.tag")
    if raw_tag is not None and not isinstance(raw_tag, str):
        problems.append(
            f"authentik.tag must be quoted. YAML read it as the "
            f"{type(raw_tag).__name__} {raw_tag!r}, which is not what you wrote — "
            f"an unquoted 2026.10 becomes 2026.1, a different release. Put the tag "
