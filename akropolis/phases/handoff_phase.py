...
class HandoffPhase(Phase):
    name = "handoff"
    # Nothing on any node is touched (see module docstring) — a typed-site-name
    ...
    def apply(self, ctx: PhaseContext) -> None:
        ...
        content = render(
            ...
            authentik_port=9443,
            # Always false, and not derived from tls.provider — see the
            # template comment. The monitor probes each node at its IP, and
            # no certificate this cluster can hold names an IP, so a `true`
            # here fails the handshake on every node regardless of who issued
            # the cert. Provenance is not what decides this; the address the
            # monitor dials is.
            verify_tls=False,
