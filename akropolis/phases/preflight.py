...
class PreflightPhase(Phase):
    ...
    def plan(self, ctx: PhaseContext) -> list[str]:
        ...
        if cfg.tls.provider != "none" and cfg.tls.hostname:
            ...
            target = "the VIP" if cfg.topology == "ha" else "THIS node's public IP"
            lines.append(f"DNS: {cfg.tls.hostname} resolves to *something* ({note}) — NAT means "
                         f"akropolis cannot verify the address it returns actually reaches "
                         f"{target}; check that by hand")
        return lines
    ...
    def apply(self, ctx: PhaseContext) -> None:
        ...
        if cfg.topology == "ha":
            ...
            # DNS. This used to demand cfg.network.vip appear in the answer,
            # hard-failing ACME otherwise. That conflates "the name reaches
            # the VIP" with "the name resolves *to* the VIP", and only the
            # first one matters: a public hostname whose A record points at a
            # public address the firewall DNATs :80/:443 to the VIP is a
            # correct, common deployment — and an RFC1918 VIP can never appear
            # in a public record at all, so the old check was unsatisfiable
            # there rather than merely premature. It also proved nothing in
            # the direct-resolution case: resolving to the VIP does not mean
            # anything answers on it.
            #
            # What genuinely kills HTTP-01 is a name that resolves to nothing
            # — a typo, or a record nobody created yet — so that stays hard
            # for acme. A VIP match is reported as information when it
            # happens. Same shape as the single-node branch below: state what
            # was observed, name what akropolis cannot see from inside, and
            # leave the NAT path to the operator.
            if cfg.tls.provider != "none" and cfg.tls.hostname:
                ...
                try:
                    ...
                    detail = f"resolves to {resolved or 'nothing'}"
                    if cfg.network.vip in resolved:
                        detail += " — the VIP itself (resolved directly, no NAT in front)"
                    elif resolved:
                        detail += (f" — not the VIP ({cfg.network.vip}); fine if that address "
                                   "forwards :80/:443 to it, verify by hand")
                    ctx.record("cluster", f"DNS {cfg.tls.hostname} resolves", bool(resolved),
                               detail, warn=(not resolved and not hard))
                except Exception as exc:  # noqa: BLE001
                    ctx.record("cluster", f"DNS {cfg.tls.hostname} resolves", False,
                               str(exc), warn=not hard)
