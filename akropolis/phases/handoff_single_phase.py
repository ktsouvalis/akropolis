...
class HandoffSinglePhase(Phase):
    ...
    def apply(self, ctx: PhaseContext) -> None:
        ...
        tls_enabled = cfg.tls.provider != "none"
        content = render(
            ...
            # The bare-metal nginx in front of the node (nginx_single_phase.py)
            # is what the public — and this probe — actually reaches, on 443
            # normally or plain 80 for tls.provider 'none' (testing only).
            authentik_port=443 if tls_enabled else 80,
            authentik_scheme="https" if tls_enabled else "http",
            # Always false, not derived from tls.provider: the monitor probes
            # the node at its IP and no cert here names an IP, so verification
            # cannot succeed whoever issued it. Same reasoning as the HA
            # handoff — see the template comment.
            verify_tls=False,
        ...
        if cfg.tls.provider == "none":
            console.print("  [yellow]NOTE: plain HTTP, no certificate at all — "
                          "testing only. WebAuthn/passkeys will not work without a "
                          "secure context.[/yellow]")
        if cfg.tls.provider == "self_signed":
            console.print("  [yellow]NOTE: serving a self-signed certificate "
                          "generated by nginx (valid 10 years) — browsers will warn "
                          "on first visit.[/yellow]")
